...
// GET insights timeline (full history)
router.get('/timeline', async (req, res) => {
  try {
    ...
    const insights = await Insight.find({ userId })
      .sort({ createdAt: -1 })
      .select('overview prediction savingsPlan microTip overspendAreas createdAt')
      .lean();

    return res.json(insights);
  } catch (err) {
    console.error("[Insights Route] Error fetching timeline:", err);
    return res.status(500).json({ error: "Failed to fetch insights timeline" });
  }
});
